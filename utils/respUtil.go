...
import (
	...
	"net/http"
	"strconv"
	"tiny-admin-api-serve/enums/sessionStatus"
	"tiny-admin-api-serve/utils/response"

	"github.com/gin-gonic/gin"
)

var (
	success          = &response.ResponseData{Code: response.Success, Msg: response.Success.Msg(), Success: true}
	parameter        = &response.ResponseData{Code: response.Parameter, Msg: response.Parameter.Msg(), Success: false}
	unauthorized     = &response.ResponseData{Code: response.Unauthorized, Msg: response.Unauthorized.Msg(), Success: false}
	forbidden        = &response.ResponseData{Code: response.Forbidden, Msg: response.Forbidden.Msg(), Success: false}
	successGzip      []byte
	parameterGzip    []byte
...
func DataPackageZip(c *gin.Context, data []byte) {
	...
	c.Header("Content-Disposition", "attachment; filename=\"tiny-amdin.zip\"")
	c.Header("Content-Length", strconv.Itoa(len(data)))
